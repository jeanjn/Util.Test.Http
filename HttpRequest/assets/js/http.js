...
.controller('HttpController', function ($scope) {
    ...
    function sendRequest(path, data, method) {
        $.ajax({
            url: $scope.cmd.service + path,
            method: method,
            headers: { Authorization: $scope.cmd.token },
            data: data,
            dataType: 'json',
            success: onSuccess,
            error: onError
    ...
    function onSuccess(data) {
        $scope.cmd.status = "Success on send request";
        try {
            var output = JsonHuman.format(data);
            $scope.cmd.result = output;
        }
        catch (e) {
            $scope.cmd.result = data;
        }
        $scope.cmd.simpleResult = data;
        $scope.$apply();
    };

    function onError(data) {
        $scope.cmd.status = "Error on send request";
        ...
        try {
            ...
            $scope.cmd.result = response;
        }
        $scope.cmd.simpleResult = response;
        $scope.$apply();
    };
